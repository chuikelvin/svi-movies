...
export default function KidsContentPage() {
  const { kidsContent, fetchMovies } = useMovieStore();
  const [currentPage, setCurrentPage] = useState(1);
  const router = useRouter();

  useEffect(() => {
    fetchMovies(currentPage, "kids");
          ...
          <>
            ...
            <div className="flex justify-center items-center gap-4 mt-8">
              ...
              <span className="text-[var(--color-text-secondary)]">
                Page {currentPage} of {kidsContent.totalPages}
              </span>
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={() =>
                  setCurrentPage((p) => Math.min(kidsContent.totalPages, p + 1))
                }
                disabled={currentPage === kidsContent.totalPages}
                className="px-4 py-2 rounded-md bg-[var(--color-primary)] text-white disabled:bg-[var(--color-background-tertiary)] disabled:cursor-not-allowed hover:bg-[var(--color-primary-hover)]"
              >
                Next
              </motion.button>
